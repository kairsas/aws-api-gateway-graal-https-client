...
```bash
aws lambda create-function --function-name aws-api-gateway-graal \
...
```bash
aws lambda invoke --function-name aws-api-gateway-graal --payload '{"resource": "/{proxy+}", "path": "/ping", "httpMethod": "GET"}' build/response.txt
